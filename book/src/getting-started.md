...
the `expect!` macro, the matcher constructors (`eq`, `lt`, `contains`, ...),
...
- `expect!(port)` captures both the value *and the source text* `port`, so a
  failure names the expression.
- `.to(eq(8080))` returns a `TestResult`. The `?` propagates a mismatch; on a
...
When `expect!(port).to(eq(8080))` fails, the test does not panic with
...
fn a_fresh_cart_is_empty_and_has_no_total() -> TestResult {
    ...
    expect!(&cart).to(is_empty())?;
    expect!(cart.iter().sum::<u32>()).to_not(gt(0))?;
    Ok(())
}
```

To assert several things about one value in a single `expect!`, combine
