...
The short version: `expect!` is slower than `assert_eq!` per call, by a
...
- `expect!(a).to(eq(b))` and `expect!(a).to(lt(b))`, the `test-better` form.

Run it with `cargo bench -p test-better --bench expect_overhead`. A typical
run on a developer laptop:

```text
expect! overhead vs the stock assert macros (10000000 iters/loop)
...
`assert_eq!` on two `u32`s compiles down to a compare and a branch. `expect!`
...
gap between `assert_eq!` and `expect!`. A test suite's wall time is dominated
...
the same `expect!` across many thousands of generated inputs. Even there the
